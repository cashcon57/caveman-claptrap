#!/usr/bin/env node
// caveman-claptrap — Claude Code SessionStart activation hook
//
// Runs on every session start:
//   1. Writes flag file at ~/.claude/.caveman-claptrap-active (statusline reads this)
//   2. Emits Claptrap ruleset as hidden SessionStart context
...
const flagPath = path.join(claudeDir, '.caveman-claptrap-active');
...
// 2. Emit full Claptrap ruleset, filtered to the active intensity level.
...
if (INDEPENDENT_MODES.has(mode)) {
  const command = mode === 'compress' ? '/claptrap:compress' : '/claptrap-' + mode;
  process.stdout.write('CLAPTRAP MODE ACTIVE — level: ' + mode + '. Behavior defined by ' + command + '.');
  process.exit(0);
}
...
// Read SKILL.md — the single source of truth for Claptrap behavior.
// Plugin installs: __dirname = <plugin_root>/hooks/, SKILL.md at <plugin_root>/skills/caveman-claptrap/SKILL.md
// Standalone installs: __dirname = ~/.claude/hooks/, SKILL.md won't exist — falls back to hardcoded rules.
let skillContent = '';
try {
  skillContent = fs.readFileSync(
    path.join(__dirname, '..', 'skills', 'caveman-claptrap', 'SKILL.md'), 'utf8'
  );
...
if (skillContent) {
  ...
  output = 'CLAPTRAP MODE ACTIVE — level: ' + modeLabel + '\n\n' + filtered.join('\n');
} else {
  // Fallback when SKILL.md is not found (standalone hook install without skills dir).
  // This is the minimum viable ruleset — better than nothing.
  output =
    'CLAPTRAP MODE ACTIVE — level: ' + modeLabel + '\n\n' +
    ...
    'Current level: **' + modeLabel + '**. Switch: `/claptrap lite|full|ultra|wenyan-lite|wenyan|wenyan-ultra`. `/caveman ...` stays as alias.\n\n' +
...
try {
  ...
  if (!hasStatusline) {
    ...
    const scriptName = isWindows ? 'caveman-claptrap-statusline.ps1' : 'caveman-claptrap-statusline.sh';
    ...
    output += "\n\n" +
      "STATUSLINE SETUP NEEDED: The Claptrap plugin includes a statusline badge showing active mode " +
      "(e.g. [CLAPTRAP], [CLAPTRAP:ULTRA]). It is not configured yet. " +
