#!/bin/bash
# caveman-claptrap — one-command hook installer for Claude Code
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the Claptrap hooks (used to merge"
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if CLAPTRAP_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.CLAPTRAP_SETTINGS, 'utf8'));
      const hasClaptrapHook = (event) =>
      ...
      process.exit(
        hasClaptrapHook('SessionStart') &&
        hasClaptrapHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Claptrap hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/caveman-claptrap-activate.js" ]; then
  echo "Reinstalling Claptrap hooks (--force)..."
else
  echo "Installing Claptrap hooks..."
fi
...
chmod +x "$HOOKS_DIR/caveman-claptrap-statusline.sh"
...
CLAPTRAP_SETTINGS="$SETTINGS" CLAPTRAP_HOOKS_DIR="$HOOKS_DIR" node -e "
  ...
  // SessionStart — auto-load Claptrap rules
  ...
  // UserPromptSubmit — track mode changes when user types /claptrap or /caveman commands
  ...
  // Statusline — wire Claptrap badge (report if skipped)
...
echo "  - SessionStart hook: auto-loads Claptrap rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/claptrap lite, /claptrap ultra, /claptrap-commit, etc.; /caveman aliases still work)"
echo "  - Statusline badge: shows [CLAPTRAP] or [CLAPTRAP:ULTRA] etc."
